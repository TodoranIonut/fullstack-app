...
Seed data (`V1.1__populate_mock_data.sql`) runs **only** on the `local` Spring profile.
...
- Strategy pattern for order processing (`SINGLE_LOCATION` vs `MOST_ABUNDANT`), configured via `app.order.strategy` in `application.yml`
...
- **Cart state is not persisted** — `CartService` uses an in-memory `BehaviorSubject`. Items are lost on page refresh; they only reach the backend when an order is placed.
- **JWT has no refresh mechanism** — tokens expire after 24 hours. Users must re-login; there is no silent refresh.
- **Mock mode requires handlers** — adding a new API endpoint without a corresponding handler in `core/mocks/interceptors/handlers/` will break `npm run start:mock`.
